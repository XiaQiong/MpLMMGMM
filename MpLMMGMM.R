...
#' @param Dem Demographic variables.
#' @param X X represents fixed effects.
...
#' @param maxiter The maximum iterations and the default is 100.
#' @param tol The tolerance for estimation during iteration process.
#' @param standardize All data will be standardize under the default setting.
#'
#' @return The prediction values and ture values for testing subjects will be returned.
#' @return The fixed and random effects sizes for each region can also returned.
#' @export
#' @import glmnet
#' @import MASS
#' @importFrom glmnet cv.glmnet

Omics_GMMLasso<-function(y,Dem=NULL,X,Gen,index,K=NULL,returnK=F, maxiter=100, tol=1e-4, standardize=TRUE){
